// g++ -std=c++17 -o multi_query_dep_cheby multi_query_dep_cheby.cpp -DOPENFHE_VERSION=1.0.3 -Wno-parentheses -DMATHBACKEND=4 -Wl,-rpath,/usr/local/lib/ /usr/local/lib/libOPENFHEcore.so /usr/local/lib/libOPENFHEpke.so /usr/local/lib/libOPENFHEpke_static.a /usr/local/lib/libOPENFHEcore_static.a -I /usr/local/include/openfhe/core -I /usr/local/include/openfhe/pke -I /usr/local/include/openfhe/ -lstdc++fs -O3 -fopenmp
...
void EvalFunctionExample() {
    std::cout << "--------------------------------- EVAL DEP CHEBYSHEV FUNCTION for 2 receiver elements ---------------------------------"
              << std::endl;
...
for (double i = 0; i < batchSize-1; ++i) {
...
// new idea ends
  ...
  //  sender_vals.push_back(test_ctext2);
    ...
    Ciphertext<DCRTPoly> result_temp_1, result_temp_2;
