...
//           processCiphertext(i, input_ciphertext_filenames, cryptoContext, tempCiphertext, resultCt);
//       }
...
for (unsigned int i = 0; i < num_ciphertexts; i++) {
    ...
    // Deserialize into tempCiphertext
    Ciphertext<DCRTPoly> *tempCiphertext;
    Ciphertext<DCRTPoly> *newCiphertextPtr;

    tempCiphertext = new Ciphertext<DCRTPoly>;
    Serial::Deserialize(*tempCiphertext, ifs, SerType::BINARY);
    ...
    #pragma omp critical
    {
        cryptoContext->EvalAddInPlace(resultCt, *newCiphertextPtr);
    }
